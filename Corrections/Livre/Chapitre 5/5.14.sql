CREATE OR REPLACE PACKAGE livre AS
  ...
  TYPE tableauEtat IS TABLE OF integer INDEX BY varchar2(2);
  FUNCTION NbExParEtat (visbn number) RETURN tableauEtat;
...
------------------- Fonction NbExParEtat
FUNCTION NbExParEtat (visbn number) 
RETURN tableauEtat AS
  vtabretour tableauEtat; 
BEGIN
  SELECT count(*) INTO vtabretour('NE') FROM ouvrages o INNER JOIN exemplaires e ON o.isbn = e.isbn 
  WHERE etat = 'NE' AND o.isbn = visbn;
   SELECT count(*) INTO vtabretour('BO') FROM ouvrages o INNER JOIN exemplaires e ON o.isbn = e.isbn 
  WHERE etat = 'BO' AND o.isbn = visbn;
   SELECT count(*) INTO vtabretour('MO') FROM ouvrages o INNER JOIN exemplaires e ON o.isbn = e.isbn 
  WHERE etat = 'MO' AND o.isbn = visbn;
   SELECT count(*) INTO vtabretour('MA') FROM ouvrages o INNER JOIN exemplaires e ON o.isbn = e.isbn 
  WHERE etat = 'MA' AND o.isbn = visbn;
  RETURN vtabretour;
END;
